...
def cnn_model_fn(features, labels, mode):
  ...
  loss = tf.losses.log_loss(labels=labels[:,:,:,0], predictions=pixel_pred[:,:,:,0], weights=labels[:,:,:,1])+0.01*l2_loss
  ...
  if mode == tf.estimator.ModeKeys.EVAL:
    eval_metric_ops = {"mse": tf.metrics.mean_squared_error(labels=labels[:,:,:,0], predictions=pixel_pred[:,:,:,0])}
    tf.summary.scalar("mse_scalar", eval_metric_ops["mse"])
    summary_eval_hook = tf.train.SummarySaverHook(
        save_steps=1,
        output_dir=SAVE_PATH+"/eval",
        summary_op=tf.summary.merge_all())
    return tf.estimator.EstimatorSpec(mode=mode, loss=loss, eval_metric_ops=eval_metric_ops, training_hooks=[summary_eval_hook]) 
